    ...
    def predict_amp_reward(
        ...
        with torch.no_grad():
            ...
            if self.use_lerp:
                ...
                # Weighted blend: r = w_G * r_task + w_S * (coef * f(d)),
                # with w_G = task_reward_lerp, w_S = 1 - task_reward_lerp.
                w_g = self.task_reward_lerp
                w_s = 1.0 - self.task_reward_lerp
                disc_term = disc_reward
                reward = w_g * task_reward.unsqueeze(-1) + w_s * disc_term
                self.train()
                return reward.squeeze(), d, (w_s * disc_term).squeeze()
