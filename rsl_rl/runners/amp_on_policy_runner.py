...
class AMPOnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if hasattr(self.base_env, "set_curriculum_rollout_horizon"):
            self.base_env.set_curriculum_rollout_horizon(self.num_steps_per_env)
    ...
    def _apply_reward_transition_hparams(self, it: int, start_iter: int, trans_iters: int) -> None:
        """During lerp warmup, optionally use a lower LR + tighter clip with fixed schedule so
        adaptive KL does not override the conservative rate (policy collapse mitigation)."""
        lr_start = self.cfg.get("transition_lr_start")
        if lr_start is None or trans_iters <= 0:
            return
        clip_start_cfg = self.cfg.get("transition_clip_start")
        if clip_start_cfg is None:
            clip_start_cfg = 0.1
        t_anchor = self.cfg.get("reward_schedule_anchor_iter")
        t0 = int(t_anchor) if t_anchor is not None else start_iter
        iters_since = it - t0
        if not hasattr(self, "_reward_transition_baselines_saved"):
            self._reward_transition_baselines_saved = True
            self._saved_alg_schedule = self.alg.schedule
            self._saved_lr_end = float(self.alg_cfg["learning_rate"])
            self._saved_clip_end = float(self.alg_cfg["clip_param"])
            if iters_since >= trans_iters:
                print(
                    f"[INFO] Reward transition window already past at resume "
                    f"(anchor_iter={t0}, trans_iters={trans_iters}); using full lr/clip."
                )
            else:
                print(
                    f"[INFO] Reward transition: iters [{t0}, {t0 + trans_iters}) — "
                    f"lr={float(lr_start):.2e} (fixed), clip={float(clip_start_cfg):.2f}; "
                    f"then lr={self._saved_lr_end:.2e} ({self._saved_alg_schedule}), "
                    f"clip={self._saved_clip_end:.2f}"
                )
        in_trans = iters_since < trans_iters
        if in_trans:
            self.alg.schedule = "fixed"
            self.alg.learning_rate = float(lr_start)
            self.alg.clip_param = float(clip_start_cfg)
        else:
            self.alg.schedule = self._saved_alg_schedule
            self.alg.learning_rate = float(self._saved_lr_end)
            self.alg.clip_param = float(self._saved_clip_end)
            if iters_since == trans_iters and not getattr(self, "_reward_transition_done_logged", False):
                self._reward_transition_done_logged = True
                print(
                    f"[INFO] Reward transition complete at iter {it} (anchor={t0}): "
                    f"lr={self.alg.learning_rate:.2e}, clip={self.alg.clip_param:.2f}, "
                    f"schedule={self.alg.schedule!r}"
                )
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        # Lerp warmup: ramp task_reward_lerp from initial disc value to target
        lerp_target = self.alg.discriminator.task_reward_lerp
        lerp_warmup_iters = self.cfg.get("lerp_warmup_iters", 0)
        lerp_start = self.cfg.get("lerp_warmup_start", lerp_target)
        if lerp_warmup_iters > 0 and lerp_start != lerp_target:
            anchor = self.cfg.get("reward_schedule_anchor_iter")
            basis0 = (start_iter - int(anchor)) if anchor is not None else 0
            if anchor is not None and basis0 >= lerp_warmup_iters:
                print(
                    f"[INFO] Lerp already at target at resume "
                    f"(anchor_iter={anchor}, warmup_iters={lerp_warmup_iters})."
                )
            else:
                print(f"[INFO] Lerp warmup: {lerp_start:.2f} → {lerp_target:.2f} "
                      f"over {lerp_warmup_iters} iterations")

        # Start training
        tot_iter = start_iter + num_learning_iterations
        for it in range(start_iter, tot_iter):
            if hasattr(self.base_env, "set_curriculum_stage"):
                self.base_env.set_curriculum_stage(it)
            # Update lerp warmup (optional absolute anchor so resume does not rewind lerp)
            if lerp_warmup_iters > 0 and lerp_start != lerp_target:
                anchor = self.cfg.get("reward_schedule_anchor_iter")
                basis = (it - int(anchor)) if anchor is not None else (it - start_iter)
                alpha = min(1.0, max(0.0, basis / lerp_warmup_iters))
                self.alg.discriminator.task_reward_lerp = lerp_start + alpha * (lerp_target - lerp_start)
            self._apply_reward_transition_hparams(it, start_iter, lerp_warmup_iters)
            start = time.time()
            # Rollout
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        self.writer.add_scalar("Train/task_reward_lerp", self.alg.discriminator.task_reward_lerp, locs["it"])
        self.writer.add_scalar("Train/clip_param", self.alg.clip_param, locs["it"])
    ...
    def save(self, path: str, infos=None):
        ...
        # -- Save discriminator and AMP normalizer
        saved_dict["disc_state_dict"] = self.alg.discriminator.state_dict()
        saved_dict["amp_normalizer"] = self.alg.amp_normalizer.__dict__
    ...
    def load(self, path: str, load_optimizer: bool = True, map_location: str | None = None,
        ...
        # -- load optimizer if used (skip when critic is reset — stale momentum)
        if load_optimizer and resumed_training and not reset_critic:
            # -- algorithm optimizer
            ...
            print("[INFO] Optimizer state NOT loaded (critic reset invalidates value-fn momentum).")
