...
from mjlab_roller.rl.config import RslRlOnPolicyRunnerCfg
...
def run_train(task_id: str, cfg: TrainConfig, log_dir: Path) -> None:
  ...
  env = env_cls(cfg=cfg.env, device=device, render_mode="rgb_array" if cfg.video else None)
  ...
  if resume_path is not None:
    ...
    load_kwargs = {}
    if hasattr(runner, 'load') and 'reset_critic' in runner.load.__code__.co_varnames:
      load_kwargs["reset_critic"] = cfg.reset_critic
    runner.load(str(resume_path), **load_kwargs)
