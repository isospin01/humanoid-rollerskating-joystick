...
def run_play(task_id: str, cfg: PlayConfig):
  ...
  skate_cmd = env_cfg.commands.get("skate")
  if cfg.command_source == "joystick" and not isinstance(skate_cmd, SkaterJoystickCommandCfg):
    raise ValueError(
      f"Task '{task_id}' does not expose a skater joystick command term; "
      "`--command-source joystick` is only supported on the new skater task."
    )
  if isinstance(skate_cmd, SkaterJoystickCommandCfg):
    skate_cmd.command_source = cfg.command_source
    skate_cmd.joystick_backend = cfg.joystick_backend
    skate_cmd.joystick_deadzone = cfg.joystick_deadzone
    skate_cmd.joystick_smoothing = cfg.joystick_smoothing
